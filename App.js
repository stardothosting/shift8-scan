...
import {Platform, StyleSheet, Text, View, AppRegistry, ScrollView, FlatList} from 'react-native';
...
// Function to scan TCP Hosts
scanTCPHost = (host,port) => {
        ...
        async.eachSeries(ipRange, function(singleIP, callback) {
          ...
          async.eachSeries(portRange, function(singlePort, callbackPort) {
            var q = async.queue(async.asyncify(async function(singlePort) {
              return await this.scanTCPHost(singleIP, singlePort);
            }));
